  ...
  try {
    ...
    while (!glfwWindowShouldClose(window)) {
     ...
     // === MURS === (+ contours)
      ...
      // AFFICHAGE HUD (SCORE)
      // ==========================
      glUseProgram(0);          // Désactive les shaders modernes
      glDisable(GL_DEPTH_TEST); // Affiche par-dessus la 3D
      glDisable(GL_LIGHTING);

      // Configuration d'une projection orthogonale (pixel-perfect)
      glMatrixMode(GL_PROJECTION);
      glPushMatrix();
      glLoadIdentity();
      ...
      if (gameFinished) {
          // Overlay de fin (Rectangle semi-transparent)
          glColor4f(0.0f, 0.0f, 0.0f, 0.6f);
          glBegin(GL_QUADS);
              glVertex2f(fbw/2 - 200, fbh/2 - 50);
              glVertex2f(fbw/2 + 200, fbh/2 - 50);
              glVertex2f(fbw/2 + 200, fbh/2 + 50);
              glVertex2f(fbw/2 - 200, fbh/2 + 50);
          glEnd();
          ...
          std::string winMsg = "FINISHED - TIME: " + std::to_string(finalTime).substr(0, 5) + "s";
          drawText(fbw/2 - 150, fbh/2 - 10, 20, winMsg.c_str());
      } else {
          // Affichage temps réel en haut à gauche
          glColor3f(1.0f, 1.0f, 1.0f); // Blanc
          std::string timeMsg = "TIME: " + std::to_string(currentTime).substr(0, 5) + "s";
          drawText(20, fbh - 40, 18, timeMsg.c_str());
      }
