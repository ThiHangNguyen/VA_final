...
GLuint link(const std::vector<GLuint>& shaders) {
  ...
  if (!ok) {
    GLint logLen = 0; glGetProgramiv(p, GL_INFO_LOG_LENGTH, &logLen);
    std::vector<GLchar> log(std::max(1, logLen));
...
uniform float uThicknessPx; uniform vec2 uViewport;
void main(){
  vec4 p0 = gl_in[0].gl_Position; vec4 p1 = gl_in[1].gl_Position;
  vec2 ndc0 = p0.xy / p0.w; vec2 ndc1 = p1.xy / p1.w;
  vec2 dir = ndc1 - ndc0; float len = length(dir);
  vec2 n = (len > 1e-6) ? normalize(vec2(-dir.y, dir.x)) : vec2(0.0, 1.0);
  vec2 px2ndc = 2.0 / uViewport; vec2 off = n * uThicknessPx * px2ndc;
  float z0 = p0.z / p0.w; float z1 = p1.z / p1.w;
  vec4 v0 = vec4(ndc0 - off, z0, 1.0);
  vec4 v1 = vec4(ndc0 + off, z0, 1.0);
  vec4 v2 = vec4(ndc1 - off, z1, 1.0);
  vec4 v3 = vec4(ndc1 + off, z1, 1.0);
  gl_Position = v0; EmitVertex(); gl_Position = v1; EmitVertex();
  gl_Position = v2; EmitVertex(); gl_Position = v3; EmitVertex(); EndPrimitive();
}
)";
